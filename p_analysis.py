...
def plot_deathrate_by_sex_and_age(y_min_a, y_min_b, ticks_interval_A, ticks_interval_B, cc):
	...
	chart_texts = get_chart_texts("deathratebysex", cc)
	...
	xaxis = plt.xlim()
	s = plt.ylim()
	cc.grid_and_ticks(y_min_a, s[1], ticks_interval_A, 1, 2)
	cc.ticks_locator(cc.e_week_interval)
	plt.gca().xaxis.set_ticklabels([])
	byagerate = plt.subplot2grid((2, 1), (1, 0))
	chart_texts = get_chart_texts("deathratebyage", cc)
	...
	s = plt.ylim()
	cc.grid_and_ticks(y_min_b, s[1], ticks_interval_B, 1, 2)
	cc.ticks_locator(cc.e_week_interval)
	cc.save_plot("deathratebysexage_evolution", f, "A")

def plot_deathrate_by_age(y_min, ticks_interval, cc):
	print("-- Plotting deathrate evolution...", end="\n")
	...
	cc.build_legend()
	s = plt.ylim()
	cc.grid_and_ticks(y_min, s[1], ticks_interval, 1, 2)
	cc.ticks_locator(cc.week_interval)
	cc.save_plot("deathratebyage_evolution", f, "A")

def plot_estimation(y_min_a, y_min_b, ticks_interval_A, ticks_divisor_A, ticks_interval_B, ticks_divisor_B, cc):
	print("-- Plotting deathrate evolution...", end="\n")
	f = figure(num=None, figsize=(cc.d_w, cc.d_h), dpi=cc.image_resolution, facecolor=cc.background_figure, edgecolor='k')
	cumestimation = plt.subplot2grid((2, 1), (0, 0))
	chart_texts = get_chart_texts("estimationcum", cc)
	...
	xaxis = plt.xlim()
	s = plt.ylim()
	cc.grid_and_ticks(y_min_a, s[1], ticks_interval_A, ticks_divisor_A, 1)
	cc.ticks_locator(cc.e_week_interval)
	plt.gca().xaxis.set_ticklabels([])
	knownratio = plt.subplot2grid((2, 1), (1, 0))
	chart_texts = get_chart_texts("knownratio", cc)
	...
	plt.xlim(xaxis[0], xaxis[1])
	s = plt.ylim()
	cc.grid_and_ticks(y_min_b, s[1], ticks_interval_B, ticks_divisor_B, 2)
	cc.ticks_locator(cc.e_week_interval)
	cc.save_plot("estimation_cum", f, "A")

def plot_estimation_avg(y_min_a, y_min_b, ticks_interval_A, ticks_divisor_A, ticks_interval_B, ticks_divisor_B, cc):
	new_end_date = pd.to_datetime(cc.end_date, format="%Y-%m-%d") - pd.Timedelta(days=30)
	print("-- Plotting deathrate evolution...", end="\n")
	f = figure(num=None, figsize=(cc.d_w, cc.d_h), dpi=cc.image_resolution, facecolor=cc.background_figure, edgecolor='k')
	estimationavg = plt.subplot2grid((2, 1), (0, 0))
	chart_texts = get_chart_texts("estimationavg", cc)
	...
	xaxis = plt.xlim()
	s = plt.ylim()
	cc.grid_and_ticks(y_min_a, s[1], ticks_interval_A, ticks_divisor_A, 0)
	cc.ticks_locator(cc.e_week_interval)
	plt.gca().xaxis.set_ticklabels([])
	knownratio = plt.subplot2grid((2, 1), (1, 0))
	chart_texts = get_chart_texts("knownratio", cc)
...
def plot_age_ratios(y_min_a, y_min_b, y_max_a, y_max_b, ticks_interval_A, ticks_interval_B, cc):
	print("-- Plotting age ratios...", end="\n")
	f = figure(num=None, figsize=(cc.d_w, cc.d_h), dpi=cc.image_resolution, facecolor=cc.background_figure, edgecolor='k')
	confirmed = plt.subplot2grid((2, 1), (0, 0))
	chart_texts = get_chart_texts("c_ageratios", cc)
	...
	chart_texts = get_chart_texts("d_ageratios", cc)
...
def plot_sex_ratios(y_min_a, y_min_b, y_max_a, y_max_b, ticks_interval_A, ticks_interval_B, cc):
	print("-- Plotting sex ratios...", end="\n")
	f = figure(num=None, figsize=(cc.d_w, cc.d_h), dpi=cc.image_resolution, facecolor=cc.background_figure, edgecolor='k')
	confirmed = plt.subplot2grid((2, 1), (0, 0))
	chart_texts = get_chart_texts("c_sexratios", cc)
	...
	deaths = plt.subplot2grid((2, 1), (1, 0))
	new_end_date = pd.to_datetime(cc.end_date, format="%Y-%m-%d") - pd.Timedelta(days=7)
	chart_texts = get_chart_texts("d_sexratios", cc)
...
def plot_origin_ratios(y_min_a, y_min_b, y_max_a, y_max_b, ticks_interval_A, ticks_interval_B, cc):
	print("-- Plotting origin ratios...", end="\n")
	f = figure(num=None, figsize=(cc.d_w, cc.d_h), dpi=cc.image_resolution, facecolor=cc.background_figure, edgecolor='k')
	confirmed = plt.subplot2grid((2, 1), (0, 0))
	chart_texts = get_chart_texts("c_origin", cc)
	new_end_date = pd.to_datetime(cc.end_date, format="%Y-%m-%d") - pd.Timedelta(days=14)
	...
	plt.gca().xaxis.set_ticklabels([])
	deaths = plt.subplot2grid((2, 1), (1, 0))
	chart_texts = get_chart_texts("d_origin", cc)
	for i in range(len(origin_groups)):
